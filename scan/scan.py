...
class Nscan:
    ...
    def makeIpPool(self, startIP, lastIP):
        ...
        intIP1 = lambda ip: int(ipaddress.IPv4Address(ip))
        ipPool1 = {ipaddress.IPv4Address(ip) for ip in range(intIP1(startIP), intIP1(lastIP)+1)}
        
        
   #     intIP = lambda ip: IPy.IP(ip).int()
    #    ipPool = {IPy.intToIp(ip, IPver) for ip in range(intIP(startIP), intIP(lastIP)+1)}

    #    return {ip for ip in ipPool1 if self.isUnIP(ip)}
    
        return {ip for ip in ipPool1 }
...
if __name__=='__main__':
    s = Nscan()
    ipPool = s.makeIpPool('192.168.1.1', '192.168.1.254')
